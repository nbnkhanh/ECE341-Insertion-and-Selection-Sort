...
### SELECTION SORT ###
SelectionSort:#$a0:size, $a1: addr of array
addi	$sp,$sp,-12
sw	$s0,8($sp)
sw	$s1,4($sp)
sw	$ra,0($sp)
...
addi	$s1,$s0,1	#i=j+1
sll	$t4,$t2,2
add	$t4,$a1,$t4	#t4=&a[iMin]
lw	$t4,0($t4)	#t4=a[iMin]
beq	$0,$0,L2
L3:
bne	$t2,$s0,swap
L6:
...
add	$t5,$t5,$a1	#t5=&a[i]
lw	$t5,0($t5)	#t5=a[i]
slt	$t3,$t5,$t4	#t3=1 if a[i]<a[iMIn]
bne	$t3,$0,L4	#t3=1 => a[i]<a[iMin] go to L4
L5:
addi	$s1,$s1,1	
L2:
slt	$t3,$s1,$a0	#t3=1 if i<n
beq	$t3,$0,L3
j 	Loop2
L4:
add	$t2,$s1,$0	#iMin=i
add	$t4,$t5,$0	#a[iMin]=a[i]
j	L5
L1:
slt	$t1,$s0,$t0	#t1=1 if j<n-1
beq	$t1,$0,Exit
j	Loop1
swap:
sll	$t4,$t2,2
